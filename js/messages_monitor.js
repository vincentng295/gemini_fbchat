...
if (window.__MESSAGE_WATCHER_LOADED__) {
    ...
    function checkNodeForIcons(node) {
        // Kiểm tra khi có sự thay đổi bên trong a[href^="/messages/"]
        // Thêm [aria-current="false"] để tránh việc kiểm tra lại thẻ a đang mở (aria-current="page")
        const parentA = node.closest('a[href^="/messages/"][aria-current="false"]');
    ...
    const observer = new MutationObserver(mutations => {
        mutations.forEach(mutation => {
            mutation.addedNodes.forEach(node => {
                ...
                // Nếu node chính là <a>
                if (node.matches?.('a[href^="/messages/"][aria-current="false"]')) {
                    evaluateATag(node);
                }
